...
function calScore(...score2:any){

   let average =  score2.reduce((prev:number,curr:number) => { console.log(prev + curr) ; return (prev + curr)
   })
...
//Question  56 : Self-Running User Profile: Create a quick, self-setup profile for a user that can tell you the user's name and age.
type profile = {
  Name: string;
  Age: number;
  Score: [];
...
function createQuickProfile(name: string, age: number, ...score: any) {

  let average = score.reduce((prev: number, curr: number) => {
    return prev + curr;
  });
...
let UserProfileObject = createQuickProfile(
  "Azan",
  19,
  17,
  71,
...
let UserProfileObject1 = createQuickProfile(
  "Ayesha",
  20,
  12,
  99,
...
function adder(Oldvalue: number) {
  return (NewPassedValue: number) => {
