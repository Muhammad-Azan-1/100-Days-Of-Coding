...
return name.map((names : string) => (`The Great ${names}`))
...
function make_sandwich(...sandwich:any){
...
function make_car(carName:string,company:string,modelYear:number,...moreInfo:string[]){
...
moreInfo.forEach(function(info){   //for each loop iterate the arry moreinfo and acess its elements inside the info
